...
transformed parameters{
  vector[N] theta;
  for (i in 1:N) {
    theta[i] = alpha[lvl[i]]+beta*x[i];  //vectorize likelihood
  }
}
model {
  ...
  y ~ poisson_log(theta);    //likelihood: normal with mean mu and sd sigma
}
generated quantities {
  int y_pred[N];
  for (i in 1:N) {
    y_pred[i] = poisson_log_rng(theta[i]);  //posterior predictive
  }
}
